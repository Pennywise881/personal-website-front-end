<template>
    <div class="container p-5 w-full md:w-4/6 h-screen">
        <div v-for="post in this.posts" class="p-6 bg-white border-2 border-gray-200 rounded-lg shadow-md mb-5">
            ...
            <button :id="post.key+'-btn-expand'" @click="this.expandCollapseText(post.key, $event)" class="inline-flex items-center text-sm px-2 py-1 md:px-3 md:py-2 font-medium text-center text-white bg-red-800 rounded-sm md:rounded-lg hover:bg-red-900 focus:ring-4 focus:outline-none focus:ring-red-300">
                Read More
...
export default{
    ...
    methods:{
        ...
        expandCollapseText(post_id, event)
        {   
            console.log("From posts.", this.section);
            // console.log(event.target.id);
            // if(event.target.id === post_id+'-btn-expand')
            // {
            ...
            //     event.target.id =  post_id+'-btn-collapse';
            //     document.getElementById(event.target.id).innerHTML = 'Read Less'
            // }
            // else if(event.target.id === post_id+'-btn-collapse')
            // {
            ...
            //     event.target.id =  post_id+'-btn-expand';
            //     document.getElementById(event.target.id).innerHTML = 'Read More'
            // }
            // console.log(post_id);
